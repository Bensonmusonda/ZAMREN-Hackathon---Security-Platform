...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!token) {
        ...
        alert('You are not logged in. Please log in to access your inbox.');
    ...
    async function fetchUserInfo() {
        try {
            const response = await fetch(`${MAIN_IDS_BASE_URL}/current_user`, { // Assuming you'll add a /current_user endpoint
                method: 'GET',
                headers: {
    ...
    async function fetchMessages(type) {
        ...
        inboxTitle.textContent = type === 'email-inbox' ? 'Your Email Inbox' : 'Your SMS Inbox';

        let url = '';
        if (type === 'email-inbox') {
            url = `${MAIN_IDS_BASE_URL}/user/emails/inbox`; // No email ID in URL
        } else if (type === 'sms-inbox') {
            url = `${MAIN_IDS_BASE_URL}/user/sms/inbox`; // No phone number in URL
        }
    ...
    // Fetch user info (assuming a /current_user endpoint is added to backend)
    await fetchUserInfo(); 
    
    // Set default to Email Inbox and fetch messages
    messageViewTypeSelect.value = 'email-inbox';
    fetchMessages('email-inbox');
